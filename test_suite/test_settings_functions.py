...
class TestSetDefaultChartSaveLocation:
    ...
    def test_set_default_chart_save_location(
        self, monkeypatch, user_setting_location, user_supplied_location
    ):
        ...
        assert (
            ...
            == test_original_chart_save_folder_location
        )
        # Expected new location:
        test_new_chart_save_location = Path(
            settings_functions.APP_DEFAULT_CHART_SAVE_DIR,
            settings_functions.CHART_SAVE_DIR_NAME,
        )  # App default if no user input.
...
class TestAppStartSetDatabase:
    @pytest.mark.parametrize("user_setting_location", [True, False])
    def test_app_start_set_database_backend(self, monkeypatch, user_setting_location):
...
class TestSetDatabaseBackend:
    ...
    def test_set_database_backend(self, monkeypatch, user_setting_backend, user_selected_backend):
        ...
        def mocked_edit_app_settings_file(new_setting):
            assert (
                new_setting["database"] == user_selected_backend
                or definitions.DEFAULT_DATABASE_BACKEND
            )
            if new_setting["database"] != (
                user_selected_backend or definitions.DEFAULT_DATABASE_BACKEND
            ):
                raise ValueError(
...
class TestCreateChartSaveFolder:
    ...
    def test_create_chart_save_folder(self, monkeypatch, test_original_location):
        ...
        monkeypatch.setattr(
            settings_functions, "move_chart_save_folder", mocked_move_chart_save_folder
        )
...
class TestMoveChartSaveFolder:
    ...
    def test_move_chart_save_folder(self, monkeypatch, original_location_exists):
        ...
        def mocked_move_file(original_location, new_location):
            assert (original_location, new_location) == (
                test_original_save_folder_path,
                test_new_save_folder_path,
            )
            move_file_mock["called"] = True

        monkeypatch.setattr(
            settings_functions.Path, "exists", lambda path: original_location_exists
        )
...
class TestWriteSettingsToFile:
    def test_write_settings_to_file(self):
        ...
        with patch(
            "dionysus_app.settings_functions.open", mock_open(read_data=None)
        ) as mocked_open:
            ...
            mocked_settings_file.write.assert_called_once_with(
                f"dionysus_settings = {test_settings_dict}"
            )
...
class TestCreateAppSettingsFile:
    ...
    def test_create_app_settings_file(self, monkeypatch, new_settings_dict, dict_to_write):
        ...
        monkeypatch.setattr(
            settings_functions, "write_settings_to_file", mocked_write_settings_to_file
        )
        monkeypatch.setattr(
            settings_functions,
            "create_app_data__init__",
...
class TestCreateAppDataInit:
    def test_create_app_data__init__(self):
        with patch(
            ...
            mocked_settings_file = mocked_open()
            mocked_settings_file.write.assert_called_once_with(
                '"""__init__.py so that settings.py may be imported."""'
            )
...
class TestEditAppSettingsFile:
    ...
    def test_edit_app_settings_file(
        self, monkeypatch, settings_file_exists, existing_settings_dict
    ):
        settings_dict_to_write = {
            "system of government": "Strange women lying in ponds distributing swords."
        }
        ...
        monkeypatch.setattr(
            settings_functions,
            "create_app_settings_file",
            mocked_create_app_settings_file,
        )
        monkeypatch.setattr(
            settings_functions, "write_settings_to_file", mocked_write_settings_to_file
        )
