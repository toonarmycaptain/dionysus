...
class TestQuitApp:
    def test_quit_app(self, monkeypatch, empty_generic_database):  # noqa: F811
        clear_temp_mock, DATABASE_close_mock, exit_mock = (
            {"called": False},
            {"called": False},
            {"called": False},
        )
        ...
        with pytest.raises(SystemExit):
            ...
            assert all(
                [
                    DATABASE_close_mock["called"],
                    exit_mock["called"],
                    clear_temp_mock["called"],
                ]
            )
...
class TestRunApp:
    def test_run_app_sets_cwd_correctly(self, monkeypatch):
        ...
        load_chart_save_folder_mock, load_database_mock = (
            {"called": False},
            {"called": False},
        )
