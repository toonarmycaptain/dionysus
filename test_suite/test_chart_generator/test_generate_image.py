...
from dionysus_app.chart_generator.generate_image import (
    add_avatar_to_plot,
    add_avatars_to_plot,
    ...
    def test_generate_chart_image(self, monkeypatch, empty_generic_database):  # noqa: F811
        ...
        class MockPlt:
            ...
            def subplots_adjust(self, left, right, top, bottom, wspace, hspace):
                assert (left, right, top, bottom, wspace, hspace) == (
                    0.05,
                    0.95,
                    0.9,
                    0.1,
...
class TestAddAvatarToPlot:
    def test_add_avatar_to_plot(self, monkeypatch):
        ...
        class MockPlt:
            def __init__(self):
                self.calls_to_mock_plt = {"imread": False, "draw": False}
...
class TestValidateAvatar:
    ...
    def test_validate_avatar(
        ...
        monkeypatch.setattr(
            generate_image, "avatar_file_exists", lambda avatar_path: avatar_path_exists
        )
        monkeypatch.setattr(generate_image.definitions, "DATABASE", test_database)
