...
class TestAssignAvatarsToBands:
    def test_assign_avatars_to_bands(self):
        test_assign_avatars_to_bands_result = {
            0: ["foo", "spam", "dead", "parrot", "halibut", "patties"],
            10: [
                "original",
                "recipe",
                "chicken",
...
class TestAssignAvatarCoords:
    def test_assign_avatar_coords(self):
        test_assign_avatar_coords_result = {
            "foo": [(0.0, 5), (10.0, 35)],
            "spam": [(0.0, 15), (10.0, 45)],
            "dead": [(0.0, 25), (10.0, 55)],
        ...
        assert (
            assign_avatar_coords(assign_avatars_to_bands(test_score_students_dict))
            == test_assign_avatar_coords_result
        )
...
class TestGenerateAvatarCoords:
    ...
    def test_generate_avatar_coords_backends(
        ...
        test_generate_avatar_coords_result = {
            ...
            **(
                {
                    test_database.get_avatar_path(
                        test_class.id, test_class.students[8].avatar_id
                    ): [(0.0, 35)],  # Ashley
                    test_database.get_avatar_path(
                        test_class.id, test_class.students[0].avatar_id
                    ): [(0.0, 5)],  # Cali
                    test_database.get_avatar_path(
                        test_class.id, test_class.students[12].avatar_id
                    ): [(10.0, 5)],  # Danielle
                }
                if isinstance(test_database, JSONDatabase)
                else {
                    test_database.get_avatar_path(test_class.students[8].avatar_id): [
                        (0.0, 35)
                    ],  # Ashley
                    test_database.get_avatar_path(test_class.students[0].avatar_id): [
                        (0.0, 5)
                    ],  # Cali
                    test_database.get_avatar_path(test_class.students[12].avatar_id): [
                        (10.0, 5)
                    ],  # Danielle
                }
            ),
        ...
        assert (
            generate_avatar_coords(test_student_scores_dict, test_class.id)
            == test_generate_avatar_coords_result
        )
    ...
    def test_generate_avatar_coords_generic_api(
        ...
        avatar_paths = [
            f"path to {student.avatar_id}"
            if student.avatar_id is not None
