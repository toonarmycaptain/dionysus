...
class TestNewChart:
    @pytest.mark.parametrize(
        "user_wants_to_save", [True, False], ids=["User saving.", "User not saving."]
    )
    @pytest.mark.parametrize(
...
class TestSetChartParams:
    def test_set_chart_params(self, monkeypatch):
        ...
        monkeypatch.setattr(
            create_chart, "get_custom_chart_options", mocked_get_custom_chart_options
        )
...
class TestGetCustomChartOptions:
    def test_get_custom_chart_options(self, monkeypatch):
        ...
        monkeypatch.setattr(
            create_chart, "take_custom_chart_options", mocked_take_custom_chart_options
        )
...
class TestUserSaveChartImage:
    ...
    def test_user_save_chart_image(self, monkeypatch, user_supplied_location):
        ...
        monkeypatch.setattr(
            create_chart,
            "get_user_save_chart_pathname",
            mocked_get_user_save_chart_pathname,
        )
        monkeypatch.setattr(
            create_chart,
            "copy_image_to_user_save_loc",
            mocked_copy_image_to_user_save_loc,
        )
    ...
    def test_copy_image_to_user_save_loc(self, monkeypatch):
        ...
        def mocked_copy_file(app_image_location, user_save_location):
            assert (app_image_location, user_save_location) == (
                test_app_image_location,
                test_user_save_location,
            )
...
class TestCreateClassSaveFolder:
    def test_create_class_save_folder(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            create_chart,
            "get_class_save_folder_path",
            mocked_get_class_save_folder_path,
        )
...
class TestGetClassSaveFolderPath:
    def test_get_class_save_folder_path(self, monkeypatch):
        monkeypatch.setattr(
            definitions, "DEFAULT_CHART_SAVE_DIR", Path("my.DEFAULT/CHART/SAVE/FOLDER")
        )
