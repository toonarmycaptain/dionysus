    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "test_database",
                {"test_database": str},
            ),  # str() used as throwaway func to return.
            pytest.param(
                "what database?",
                {"not found": "not a database object"},
                marks=pytest.mark.xfail,
            ),
        ],
    )
    def test_load_database(
        self, monkeypatch, mock_default_database_setting, mock_database_backends
    ):
        monkeypatch.setattr(database_functions, "database_backends", mock_database_backends)
        # Mock out app_data/settings where it doesn't exist ->
        # dir/package will not exist in testing.
