...
from test_suite.testing_class_data import (
    testing_registry_data_set as test_registry_data_set,  # noqa: F401
...
def empty_json_test_db(path):
    return JSONDatabase(
        app_data_path=Path(path, "app_data"),
...
class TestGetClasses:
    ...
    def test_get_classes(self, empty_json_database, registry_list, returned_value):
...
class TestClassNameExists:
    ...
    def test_class_name_exists(
        self, empty_json_database, test_class_name, registry_list, returned_value
    ):
        test_json_database = empty_json_database
...
class TestCreateClass:
    def test_create_class(self, empty_json_database, test_full_class):  # noqa: F811
        ...
        (
            _setup_class_mock,
            _write_classlist_to_file_mock,
            _copy_avatars_to_class_data_mock,
        ...
        test_json_database._move_avatars_to_class_data = (
            mocked__copy_avatars_to_class_data
        )

        assert test_json_database.create_class(test_full_class) is None
        assert all(
            [
                _setup_class_mock["called"],
    ...
    def test_load_class(self, monkeypatch, empty_json_database, test_full_class):  # noqa: F811
        ...
        def mock_from_file(class_path):
            assert class_path == Path(
                test_json_database.class_data_path,
                test_class_name,
        ...
        assert (
            test_json_database.load_class(test_class_name).json_dict()
            == test_loaded_class
        )
...
class TestUpdateClass:
    def test_update_class(self, empty_json_database, test_full_class):  # noqa: F811
        ...
        assert (
            test_class.json_dict() == test_class_new_class.json_dict()
        ...
        for student in test_loaded_class["students"]:
            student["id"] = student["name"]
        assert (
            test_json_database.load_class(test_class.name).json_dict()
            == test_loaded_class
        )
        ...
        assert (
            new_student not in test_class and new_student.name not in test_class
        )  # Confirm student not in class.
...
class TestCreateChart:
    def test_create_chart(self, monkeypatch, tmp_path, empty_json_database):
        ...
        test_filename = (
            ...
            + empty_json_database.chart_data_file_type
        )
        test_file_folder = tmp_path.joinpath(
            test_chart_data_dict["class_name"], "chart_data"
        )
        ...
        def mocked__store_students_as_student_names(file_chart_data_dict):
            if file_chart_data_dict != test_chart_data_dict:
                raise ValueError(
                    "The dict of chart data did not contain expected items."
                )
            ...
            if file_chart_data_dict is test_chart_data_dict:
                raise ValueError(
                    "A reference to the original chart data dict was passed. \n"
        ...
        test_json_database._store_students_as_student_names = (
            mocked__store_students_as_student_names
        )
...
class TestSaveChartImage:
    def test_save_chart_image(self, empty_json_database):
        ...
        test_data_dict = {
            ...
            "score-avatar_dict": {
                0: [Path("path to Cali_avatar.png")],
                1: [
                ...
                6: [
                    Path("path to Danielle_avatar.png"),
                    test_json_database.default_avatar_path,
                ],
        ...
        assert (
            test_json_database.save_chart_image(test_data_dict, mocked_mpl_plt)
...
class TestGetAvatarPath:
    ...
    def test_get_avatar_path(self, empty_json_database, test_student_avatar):
        ...
        def mocked__avatar_path_from_string(class_name, student_avatar_filename):
            if not test_student_avatar:
                raise ValueError(
                    "Should not be called as not avatar to get abs path for."
                )
        ...
        assert test_json_database.get_avatar_path(
            "some class", test_student_avatar
        ) == (
            test_student_avatar
            if test_student_avatar
...
class TestAvatarPathFromString:
    def test__avatar_path_from_string(self, empty_json_database):
        ...
        test_avatar_path = test_json_database.class_data_path.joinpath(
            test_class_name, "avatars", test_avatar_filename
        )

        assert (
            test_json_database._avatar_path_from_string(
                test_class_name, test_avatar_filename
            )
            == test_avatar_path
        )
...
class TestClose:
    def test_close(self, empty_json_database):
        ...
        test_json_database = empty_json_database
        test_json_database._registry.check_registry_on_exit = (
            mock__registry_check_registry_on_exit
        )
...
class TestSetupClass:
    def test__setup_class(self, empty_json_database):
        ...
        _setup_class_data_storage_mock, registry_register_class_mock = (
            {"called": False},
            {"called": False},
        )
...
class TestSetupClassDataStorage:
    def test_setup_class_data_storage(self, empty_json_database):
        ...
        assert test_json_database.class_data_path.joinpath(
            test_class_name, "avatars"
        ).exists()
        assert test_json_database.class_data_path.joinpath(
            test_class_name, "chart_data"
        ).exists()
        assert test_json_database.default_chart_save_dir.joinpath(
            test_class_name
        ).exists()
...
class TestWriteClasslistToFile:
    def test_write_classlist_to_file(self, empty_json_database, test_full_class):  # noqa: F811
        test_json_database = empty_json_database
        test_class_data_file_path = Path(
            test_json_database.class_data_path,
...
class TestMoveAvatarsToClassData:
                ...
                NewClass.from_dict(
                    {
                        "name": "test_class",
                        "students": [
                            {"name": "Cali", "avatar_id": "Cali_avatar.png"},
    ...
    def test_move_avatars_to_class_data(
        self, empty_json_database, test_new_class, expected_moved_avatars
    ):
...
class TestMoveAvatarToClassData:
    def test_move_avatar_to_class_data(self, monkeypatch, empty_json_database):
        ...
        assert (
            test_json_database._move_avatar_to_class_data(test_class, test_filename)
            is None
        )
    ...
    def test_move_avatar_to_class_data_avatar_preexisting(
        ...
        assert (
            test_json_database._move_avatar_to_class_data(
                test_class, destination_avatar_path.name
            )
            is None
        )
...
class TestStoreStudentsAsStudentNames:
    def test_store_students_as_student_names(self, empty_json_database):
        ...
        assert (
            test_json_database._store_students_as_student_names(test_data_dict)
            == test_returned_sanitised_data_dict
        )
