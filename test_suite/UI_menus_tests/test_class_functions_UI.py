...
class TestBlankClassDialogue:
    @pytest.mark.parametrize("ask_user_bool_return", [True, False])
...
class TestCreateChartWithNewClassDialogue:
    @pytest.mark.parametrize(
        ...
        [
            ...
            for good_input, return_value in [
                ("n", False),
                ("N", False),
                ("y", True),
                ("Y", True),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            for good_input, return_value in [
                (
                    str(key),
                    test_registry_data_set["enumerated_dict"][key],
                )  # Numerical inputs
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            for good_input, return_value in [
                (
                    str(key),
                    test_class_data_set["enumerated_dict"][key],
                )  # Numerical inputs
...
class TestSelectAvatarFileDialogue:
    def test_select_avatar_file_dialogue(self, monkeypatch):
        ...
        def mocked_select_file_dialogue(title_str, filetypes, start_dir):
            if (title_str, filetypes, start_dir) != (
                test_dialogue_box_title,
                test_filetypes,
                test_start_dir,
            ):
                raise ValueError(
