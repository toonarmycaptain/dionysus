...
class TestCleanForFilename:
    def test_clean_for_filename_mocking_call(
        ...
        monkeypatch.setattr(
            UI_functions, "scrub_candidate_filename", mocked_scrub_candidate_filename
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            " not the spanish inquisition ",
            " not the spanish inquisition",
        ),  # test_sentence_leading_and_trailing_spaces
        (
            " because nobody_expects_the _spanish_ inquisition the 2nd time",
            " because nobody_expects_the _spanish_ inquisition the 2nd time",
...
class TestSaveAsDialogue:
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                {},
                {
                    "filetypes": "should be the default",
    ...
    def test_save_as_dialogue(
        ...
        def mocked_filedialog_asksaveasfilename(
            title,
            defaultextension,
            filetypes,
            initialfile,
        ...
        monkeypatch.setattr(
            ...
            "asksaveasfilename",
            mocked_filedialog_asksaveasfilename,
        )
...
class TestSelectFileDialogue:
    ...
    def test_select_file_dialogue(
        ...
        def mocked_filedialog_askopenfilename(
            title,
            filetypes,
            initialdir,
        ):
        ...
        monkeypatch.setattr(
            UI_functions.filedialog,
            "askopenfilename",
            mocked_filedialog_askopenfilename,
        )
...
class TestSelectFolderDialogue:
    ...
    def test_select_folder_dialogue(
        self,
        mock_tk,
        monkeypatch,
        ...
        def mocked_filedialog_askdirectory(
            title,
            initialdir,
        ):
            """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "prompt",
                lambda x: not x % 2,
                lambda x: f"{x} is not even.",
                [1, 3, 5, 6],
            ),  # func error message
            (
                "prompt",
                lambda x: "a" in x,
                lambda x: f'{x} has no "a".',
                ["b", "c", "a"],
            ),  # func error message
