...
from dionysus_app.class_functions import (
    avatar_file_exists,
...
class TestCreateClasslist:
    def test_create_classlist(
        ...
        monkeypatch.setattr(
            ...
            mocked_take_classlist_name_input,
        )
        monkeypatch.setattr(
            class_functions,
            "compose_classlist_dialogue",
            mocked_compose_classlist_dialogue,
        )
        ...
        monkeypatch.setattr(
            class_functions,
            "create_chart_with_new_class",
            mocked_create_chart_with_new_class,
        )
...
class TestComposeClasslistDialogue:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [Class(name="Empty class"), Class(name="Empty class")],
                [False, True],
            ),  # Empty class refused then accepted.
        ],
    )
...
class TestTakeClassDataInput:
    def test_take_class_data_input(self, monkeypatch):
        ...
        test_class = Class(
            ...
            students=[
                Student(
                    name=test_take_student_name_input_returns[0],
        ...
        def mocked_take_student_avatar(new_class, student_name):
            if (new_class.name, student_name) != (
                test_class_name,
                test_take_student_name_input_returns[0],
            ):
                raise ValueError
...
class TestCreateChartWithNewClass:
    @pytest.mark.parametrize(
        "test_classname, chose_to_create_chart_from_class",
        [
            ("Class choosing to create chart immediately", True),
            ("Class choosing not to create chart immediately", False),
        ],
    )
    def test_create_chart_with_new_class(
        self, monkeypatch, test_classname, chose_to_create_chart_from_class
    ):
...
class TestSelectClasslist:
    def test_select_classlist(self, monkeypatch):
        ...
        monkeypatch.setattr(
            class_functions, "create_class_list_dict", mocked_create_class_list_dict
        )
        monkeypatch.setattr(
            class_functions,
...
class TestCreateClassListDict:
    @pytest.mark.parametrize(
        ...
        [
            (
                test_registry_data_set["enumerated_dict"].values(),
                test_registry_data_set["enumerated_dict"],
            ),
            pytest.param(
                None,
                "error raised!",
                marks=pytest.mark.xfail('ValueError("No Database found.")'),
            ),
        ],
    )
    def test_create_class_list_dict_patching_REGISTRY(
        self,
        monkeypatch,
        empty_generic_database,  # noqa: F811
        classes_list,
...
class TestSelectStudent:
    @pytest.mark.parametrize(
        "selected_student_name, selected_student_students_index",
        [
            ("one", 0),
            ("two", 1),
            ("three", 2),
    ...
    def test_select_student(
        self, monkeypatch, selected_student_name, selected_student_students_index
    ):
        test_class_students = [
            Student(name="one"),
            Student(name="two"),
